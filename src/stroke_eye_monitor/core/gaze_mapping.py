...
class GazeCalibration:
    ...
    def to_dict(self) -> dict[str, Any]:
        if self._pipeline is not None:
            ...
            d["dwell_ms"] = int(self.dwell_ms)
            d["blink_burst_window_s"] = float(self.blink_burst_window_s)
            d["blink_commit_pause_s"] = float(self.blink_commit_pause_s)
            d["blink_idle_abort_s"] = float(self.blink_idle_abort_s)
            return d
        return {
            "version": 8,
            "gaze_width": self.gaze_width,
